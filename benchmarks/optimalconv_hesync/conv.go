...
// sortedRotations returns the keys of m in ascending order. Using a sorted
// iteration instead of the default map-range order is required for HESync:
// the tracing pass and the subsequent HESync run must invoke the evaluator's
// operations in the same sequence, so every plan instruction at opIndex N
// refers to the same underlying op in both passes.
func sortedRotations(m map[int][]int) []int {
	ks := make([]int, 0, len(m))
	for k := range m {
		ks = append(ks, k)
	}
...
func gen_idxNlogs(ECD_LV int, keygen rlwe.KeyGenerator, sk *rlwe.SecretKey, encoder ckks.Encoder, params ckks.Parameters) (idx []*ckks.Plaintext, pack_eval ckks.Evaluator) {
	idx, pack_eval, _, _ = gen_idxNlogs_keys(ECD_LV, keygen, sk, encoder, params)
	return
}

// gen_idxNlogs_keys is like gen_idxNlogs but also returns the parameters and
// rotation-key set used by the pack evaluator, so callers (e.g. HESync setup)
// can manage those keys externally.
func gen_idxNlogs_keys(ECD_LV int, keygen rlwe.KeyGenerator, sk *rlwe.SecretKey, encoder ckks.Encoder, params ckks.Parameters) (idx []*ckks.Plaintext, pack_eval ckks.Evaluator, packParams ckks.Parameters, packRtks *rlwe.RotationKeySet) {
	logN := params.LogN()
	N := params.N()
	gals := []uint64{}
	...
	packRtks = keygen.GenRotationKeys(gals, sk)
	pack_eval = ckks.NewEvaluator(params, rlwe.EvaluationKey{Rtks: packRtks})
	packParams = params

	return idx, pack_eval, packParams, packRtks
}
...
func bsgs_ctxt(eval ckks.Evaluator, encoder ckks.Encoder, input *ckks.Ciphertext, m_idx, r_idx map[int][]int, params ckks.Parameters) (result *ckks.Ciphertext) {
	...
	for _, rot := range sortedRotations(m_idx) {
		elt := m_idx[rot]
	...
	st = true
	for _, rot := range sortedRotations(r_idx) {
		elt := r_idx[rot]
		tmp := make([]complex128, params.Slots())
		for i := range elt {
			tmp[i] = complex(float64(elt[i]), 0)
