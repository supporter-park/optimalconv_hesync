...
// hesyncSkipDecrypt, when true, causes resnet test functions to skip their
// final Decrypt/writeTxt block. Used during the HESync tracing pass, where
// intermediate ciphertexts are tracer placeholders without real values.
var hesyncSkipDecrypt bool
...
type context struct {
	...
	// Retained for HESync wiring.
	rlk        *rlwe.RelinearizationKey
	rotkeys    *rlwe.RotationKeySet // main non-boot evaluator's keys
	packParams ckks.Parameters      // parameters used by pack_evaluator
	packRtks   *rlwe.RotationKeySet // pack_evaluator's rotation keys (no rlk)
	btpRtks    *rlwe.RotationKeySet // bootstrapping rotation keys (shared across btp,btp2..btp5)
	btpParams  *ckks.BootstrappingParameters
}
...
func newContext(logN, ker_wid int, in_wids, kp_wids []int, boot bool, kind string) *context {
	...
	if !(kind == "BL_Conv") {
		...
		cont.pl_idx, cont.pack_evaluator, cont.packParams, cont.packRtks = gen_idxNlogs_keys(0, new_kgen, sk, new_encoder, new_params)
	}
	...
	if boot {
		...
		cont.btpRtks = rotkeys
		cont.btpParams = btpParams
...
func main() {
	...
	if resnet {
		...
		if wide_case == 1 {
			// HESync-wrapped run (replaces the plain testResNet_crop_sparse path).
			testResNet_crop_sparse_hesync(0, test_num, ker_wid, depth, debug, cf100)
		} else if (wide_case == 2) || (wide_case == 3) {
			testResNet_crop_sparse_wide_hesync(0, test_num, ker_wid, depth, wide_case, debug, cf100)
