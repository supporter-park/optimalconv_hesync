...
func (b *BootstrappingParameters) Params() (p Parameters, err error) {
	...
	Qi = append(Qi, b.RotateModuli.Qi...)
	Qi = append(Qi, b.ReLUEvalModuli.Qi...)
...
func (b *BootstrappingParameters) Copy() *BootstrappingParameters {
	...
	// RotateModuli
	paramsCopy.RotateModuli.Qi = make([]uint64, len(b.RotateModuli.Qi))
	copy(paramsCopy.RotateModuli.Qi, b.RotateModuli.Qi)
	paramsCopy.RotateModuli.ScalingFactor = b.RotateModuli.ScalingFactor

	// ReLUModuli
	paramsCopy.ReLUEvalModuli.Qi = make([]uint64, len(b.ReLUEvalModuli.Qi))
	copy(paramsCopy.ReLUEvalModuli.Qi, b.ReLUEvalModuli.Qi)
	paramsCopy.ReLUEvalModuli.ScalingFactor = b.ReLUEvalModuli.ScalingFactor
...
type RotateModuli struct {
	Qi            []uint64
	ScalingFactor float64
}

// moduli used for ReLU (Added by DWKim)
type ReLUEvalModuli struct {
	Qi            []uint64
	ScalingFactor float64
}
...
func (b *BootstrappingParameters) MaxLevel() int {
	return len(b.ResidualModuli) + len(b.CoeffsToSlotsModuli.Qi) + len(b.RotateModuli.Qi) + len(b.ReLUEvalModuli.Qi) + len(b.SineEvalModuli.Qi) + len(b.SlotsToCoeffsModuli.Qi) - 1
}
...
var DefaultBootstrapParams = []*BootstrappingParameters{
	...
	// SET VI (III modified) - added by DWKim (CtoS, EvalSine, ReLU, StoC)
	// 1553
	{
		LogN:     16,
		LogSlots: 15,
	...
	// SET VII (fast eval) - added by DWKim (CtoS, EvalSine, ReLU, StoC)
	// 1553
	{
		LogN:     16,
		LogSlots: 15,
		...
		RotateModuli: RotateModuli{
			Qi: []uint64{
				// 0xffffffffffc0001, // 60, LV = 4
				// 0x1000002001, // 36 LV 4
				// 0x200000e001, // 37 LV 4
				// 0x3ffffea001, // 38 LV 4
				// 0x7fffe60001, // 39 LV = 4
				// 0x10000140001, // 40 LV = 4
				0x3ffffe80001, //42 LV = 4
				// 0x1fffffff50001, // 49 LV = 4
				// 0x3fde0001, // 30, LV = 4
			},
			ScalingFactor: 1 << 30,
		},
		ReLUEvalModuli: ReLUEvalModuli{
			Qi: []uint64{
				0x3ffc0001, // 30, LV = 5
				0x40080001,
				0x3fac0001,
	...
	// SET VIII (fast eval) - added by DWKim (for BL) [Similar to III]
	// 1553
	{
		LogN:     16,
		LogSlots: 15,
		...
		ResidualModuli: []uint64{
			...
			0x40b00001,
			0x40c20001, // LV = 12
			// 0x40020001, // 30 	for rescale to set scale = 30
			0xffffffffffc0001, // 60	Lv = 13 // to set the scale 30?
			// 0x10000000b01, // 40 Qi
			// 0x3fde0001,
			// 0x3fd20001,
			// 0x3fac0001,
			...
			// 0x40b00001,
			// 0x40c20001,
			// 0xffffffffffc0001, // 60	for rescale
			// 0x10000000006e0001, // 60
			// 0xfffffffff840001,  // 60
			// 0x1000000000860001, // 60
			// 0xfffffffff6a0001,  // 60
